...
// Editar
if (editBtn && e.target.classList.contains('edit-icon')) {
  ...
  if (li.classList.contains('editando')) {
    const editedName = nameEdit.textContent.trim();
    const editedNumber = numberEdit.textContent.trim();
    const nameValid = NAME_REGEX.test(editedName);
    const numberValid = NUMBER_REGEX.test(editedNumber);
    if (!nameValid || !numberValid) {
      alert("Nombre o número de contacto no válidos");
      return;
    }
    ...
    contacts = contacts.map(contact => {
      if (contact.id === li.id) {
        return {...contact, name: editedName, number: editedNumber};
      } else {
        return contact;
      }
    });
